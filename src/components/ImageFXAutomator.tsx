...
export const ImageFXAutomator = () => {
  ...
  useEffect(() => {
    chrome.storage.local.get(['imagefxPrompts', 'imagefxDownloadMode', 'imagefxProcessingState'], (result: {
      imagefxPrompts?: string;
      imagefxDownloadMode?: DownloadMode;
      imagefxProcessingState?: ProcessingState;
    }) => {
      if (result.imagefxPrompts) setPrompts(result.imagefxPrompts);
      if (result.imagefxDownloadMode) setDownloadMode(result.imagefxDownloadMode);
      if (result.imagefxProcessingState) setProcessingState(result.imagefxProcessingState);
    });

    const handleStorageChange = (changes: { [key: string]: chrome.storage.StorageChange }) => {
      if (changes.imagefxProcessingState?.newValue) {
        setProcessingState(changes.imagefxProcessingState.newValue as ProcessingState);
      }
    };
  ...
  const savePrompts = (value: string) => {
    setPrompts(value);
    chrome.storage.local.set({ imagefxPrompts: value });
  };

  const handleDownloadModeChange = (mode: DownloadMode) => {
    setDownloadMode(mode);
    setIsDropdownOpen(false);
    chrome.storage.local.set({ imagefxDownloadMode: mode });
  };
  ...
  const handleStart = async () => {
    ...
    if (!tab?.url?.includes('labs.google')) {
      chrome.tabs.create({ url: 'https://labs.google/fx/pt/tools/image-fx' });
      return;
    }
    ...
    await chrome.storage.local.set({
      imagefxProcessingState: initialState,
      imagefxShouldStop: false
    });
  ...
  const handleStop = async () => {
    await chrome.storage.local.set({
      imagefxShouldStop: true,
      imagefxProcessingState: { ...processingState, status: 'paused', isRunning: false }
    });
  ...
  const handleClear = () => {
    ...
    chrome.storage.local.remove(['imagefxPrompts', 'imagefxProcessingState', 'imagefxShouldStop']);
  };
  ...
  return (
    <div className="flex flex-col h-full gap-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      {/* CARD PRINCIPAL - ESTILO GLASS/DARK */}
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl backdrop-blur-sm shadow-xl p-4 flex flex-col gap-4">
        ...
        <div>
          ...
          <a
            href="https://labs.google/fx/pt/tools/image-fx"
            target="_blank"
      ...
      {/* FOOTER ACTIONS - BOTÕES VIBRANTES */}
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              ...
              chrome.storage.local.remove(['imagefxProcessingState', 'imagefxShouldStop']);
            }}
            disabled={processingState.isRunning}
